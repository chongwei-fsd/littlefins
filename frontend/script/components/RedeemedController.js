class RedeemedController {
    ...
    displayRedeemedVouchers(data) {
        // document.getElementById('noVoucherText').style.display='none'
        data.forEach(v => {
    ...
    addRedeemedVoucher(voucher) {
        ...
        useNowBtn.className = "redeem-btn cursorPointer";
        useNowBtn.innerText = "Use now";
        useNowBtn.onclick = () => this.handleUseNowClick(voucher.id, useNowBtn);
    ...
    handleUseNowClick(voucherId, useNowBtn) {
        useNowBarcode(voucherId);
        // useNowBtn.className="redeemed-grey-btn"
